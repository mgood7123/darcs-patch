...
int main() {
    auto p1 = makeNamedWithType_T("p1", makeAddFile());
    auto p2 = makeNamedHunk_T("p2", 1, "", "all\nthe\nlines");
    auto p3 = makeNamedHunk_T("p3", 1, "", "foo");
    auto p4 = makeNamedHunk_T("p4", 4, "", "jar");
    PatchInfo_T tmp1 = makePatchInfo_T("p1");
    PatchInfo_T tmp2 = makePatchInfo_T("p2");
    ...
    compare_print_all2(tmp2, tmp1, PatchInfo_T);
    std::cout << "calling depsGraph with p1 = " << p1 << ", p2 = " << p2 << ", p3 = " << p3 << ", p4 = " << p4 << "\n";
    DepsGraph_T graph = depsGraph_T({p1, p2, p3, p4});
    renderDepsGraphAsDot(graph);
    return 0;
}